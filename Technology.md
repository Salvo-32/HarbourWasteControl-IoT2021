...
## Kriging method
In particular **AWS EC2** execute the Kriging method, is a regression method developed by Danie Krige in 1950 to forsee underground mineral distribution, starting from already known underground samples. This method allows to interpolate a physical quantity, in our scenario pH and Turbidity, minimizing mean square error. In other words it takes as input a regression function in conjunction with the residual (namely the difference between the real estimated value and predicted value) and tries to minimize the error to give a better fit to the data.
...
To recap, Using Krigin we exploit the real collected data from sensors to determine pH, temperature and Turbidity value in nearby points of the harbour water surface where there are no sensors
